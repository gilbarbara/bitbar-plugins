...
function formatIssue(issue) {
  ...
  return [
    ...
    `${ICONS[issuetype.name]} ${issuetype.name} ‣ ${status.name}${SHOW_ALL_ISSUES ? ` (${assignee.displayName})` : ''} | color=${COLORS[issuetype.name]}`,
...
const getItems = () => {
  const query = [
    ...
    .filter(d => !!d)
    .join(' AND ');
  const fields = ['summary', 'issuetype', 'status', 'timetracking', 'assignee'];
